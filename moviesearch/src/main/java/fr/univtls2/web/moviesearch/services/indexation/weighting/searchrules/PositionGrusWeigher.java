...
public class PositionGrusWeigher implements SearchWeigher {
	...
	public void weight(final SourceDoc doc) {
		...
		for (int id = 0; id < positionsList.size() - 1; id++) {
			...
			final List<List<Integer>> listToTest = positionsList.subList(
					id + 1, positionsList.size());
			found = found
					+ searchNumberTerm(positionsList.get(id),
							listToTest);
		}
	 ...
	 * 
	 * @param first
	 *            list of position in the list of the lists
	 * @param listToTest
	 *            next lists
	 * @return number of term found
	 */
	private int searchNumberTerm(final List<Integer> first,
			final List<List<Integer>> listToTest) {
		int found = 0;
	...
	private int searchTerms(Integer position, List<Integer> positions) {
		...
		for (Integer termP : positions) {
			...
			if ((termP - MAX_DISTANCE) < position
					&& (termP + MAX_DISTANCE) > position) {
				found++;
